function isSyncFilterShape(filters) {
...
function includesNormalizedValue(candidate, allowedValues) {
    if (!allowedValues?.length)
        return true;
    const normalizedCandidate = normalizeContactIdentifier(candidate);
    return allowedValues.some((value) => normalizeContactIdentifier(value) === normalizedCandidate);
}
function includesExactValue(candidate, allowedValues) {
    if (!allowedValues?.length)
        return true;
...
export function resolveEffectiveTimeAfter(filters, now = new Date()) {
    ...
    const backfillAfter = new Date(now.getTime() - backfillDays * 24 * 60 * 60 * 1000);
    if (!explicitAfterDate || backfillAfter > explicitAfterDate) {
        return backfillAfter.toISOString();
    }
    return explicitAfterDate.toISOString();
}
export function matchesIMessageFilters(metadata, filters, now = new Date()) {
    if (!filters)
        return true;
