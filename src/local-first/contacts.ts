...
export interface LocalContactsSyncBatch {
  ...
  stats: { extracted: number; chunks: number; dbType: "contacts" };
}
...
export function buildLocalContactsSyncBatch({
  rows,
  cursor,
}: {
  ...
  const norm: Required<LocalContactsCursor> = {
    lastSyncTimestamp: cursor?.lastSyncTimestamp ?? 0,
  };

  const files: LocalContactFile[] = [];
