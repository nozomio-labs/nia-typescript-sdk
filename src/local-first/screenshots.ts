...
export interface LocalScreenshotsSyncBatch {
  ...
  stats: { extracted: number; chunks: number; dbType: "screenshots" };
}

export function buildLocalScreenshotsSyncBatch({
  rows,
  cursor,
}: {
  ...
  const norm: Required<LocalScreenshotsCursor> = {
    lastCaptureTimestamp: cursor?.lastCaptureTimestamp ?? 0,
  };

  const files: LocalScreenshotFile[] = [];
